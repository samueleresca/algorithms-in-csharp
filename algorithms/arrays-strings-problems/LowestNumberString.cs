namespace HandsOn.Algorithms.ArraysAndStrings
{
    public static class LowestNumberString
    {
        public static string RemoveKdigits(string num, int k)
        {
            ...
            if (result.StartsWith("0")) result = result.Substring(1, result.Length - 1);

            return result == "" ? "0" : result;
        }


        private static void buildLowestNumberRec(string str, int n, ref string result)
        {
            ...
            var new_str = str.Substring(minIndex + 1, len - minIndex - 1);
